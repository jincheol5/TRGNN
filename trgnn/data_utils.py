...
class DataUtils:
    ...
    @staticmethod
    def save_graph_list_to_selected_dataset_list(graph_list:list,num_nodes:int,dir_type:Literal['train','val','test']):
        dataset_list=[]
        for graph_id,graph in tqdm(enumerate(graph_list),desc=f"Convert {dir_type}_{num_nodes} graph_list..."):
            ...
            selected_node_list=random.sample(node_list,10) # 랜덤하게 source node 선택
            for source_id in tqdm(selected_node_list,desc=f"Convert {graph_id} graph to dataset..."):
    ...
    @staticmethod
    def save_graph_list_to_selected_dataset_list_chunk(graph_list:list,graph_type:str,num_nodes:int,chunk_size:int,dir_type:Literal['train','val','test']):
        dataset_list=[]
