...
class MemoryUpdater(nn.Module):
    ...
    def forward(self,x,memory,source:torch.Tensor,target:torch.Tensor,delta_t:torch.Tensor):
        ...
        Input:
            ...
            source: [B,1]
            target: [B,1]
            delta_t: [B,N,latent_dim]
        Output:
            updated_memory
        """
        batch_size=x.size(0)
        
        source_batch_indices=torch.arange(batch_size,device=x.device) # [B,]
        source=source.squeeze(-1) # [B,]
        source_memory=memory[source] # [B,latent_dim]
        source_x=x[source_batch_indices,source,:] # [B,1]
        source_delta_t=delta_t[source_batch_indices,source,:] # [B,latent_dim]
        ...
        target_x=x[target_batch_indices,target,:] # [B,1]
        target_delta_t=delta_t[target_batch_indices,target,:] # [B,latent_dim]

        source_msg_input=torch.cat([source_memory,target_memory,source_delta_t,source_x],dim=-1) # [B,latent_dim+latent_dim+latent_dim+node_dim]
        source_msg=self.src_mlp(source_msg_input) # [B,latent_dim]

        target_msg_input=torch.cat([target_memory,source_memory,target_delta_t,target_x],dim=-1) # [B,latent_dim+latent_dim+latent_dim+1]
...
class TimeProjection(nn.Module):
    ...
    def forward(self,target_memory,delta_t):
        ...
        z=torch.mul(delta_t_vec,target_memory) # [B,latent_dim]
        return z
...
class GraphAttention(nn.Module):
    ...
    def forward(self,tar_vec,h,neighbor_mask,tar_idx=None):
        """
        Input:
            tar_idx: [B,1]
            tar_vec: [B,node_dim+latent_dim+latent_dim], target node feature||time_feature
            h: [B,N,node_dim+latent_dim+latent_dim], all node feature||time_feature
